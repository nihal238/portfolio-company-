...
export function BtnPrimary({ children, onClick, type = "button", disabled = false, style = {}, ...rest }) {
  const [h, setH] = useState(false);
  const handleEnter = () => { if (!disabled) setH(true); };
  const handleLeave = () => { if (!disabled) setH(false); };
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: h ? "0 16px 40px rgba(108,99,255,0.3)" : "none",
        transition: "all 0.2s",
        ...style,
      }}
      {...rest}
